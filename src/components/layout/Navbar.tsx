...
export default function Navbar() {
  ...
  React.useEffect(() => {
    ...
    setElevate(scroll > SCROLL_THRESHOLD);
  ...
  return (
    <>
      <motion.header
        ...
        className="fixed top-0 left-0 right-0 h-20 flex items-center justify-center text-text z-50 data-[elevate=true]:bg-surface/90 data-[elevate=true]:backdrop-blur-sm transition-[background-color,backdrop-filter] duration-300"
      >
        <Container>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            className="md:hidden fixed inset-0 h-dvh w-full bg-text/80 backdrop-blur-sm z-60"
          />
        )}
        {menuOpen && (
          <motion.div
